...
COPY smartrecipebot ./smartrecipebot
COPY data ./data
...
CMD ["uvicorn", "smartrecipebot.main:application", "--host", "0.0.0.0", "--port", "8000"]
...
COPY data ./data
COPY scripts ./scripts
COPY smartrecipebot/infrastructure/knowledge/recipe_kg ./smartrecipebot/infrastructure/knowledge/recipe_kg

ENV PYTHONPATH=/var/lib/neo4j/build
RUN mkdir -p ./import_generated && \
    rm -rf ./smartrecipebot/infrastructure/knowledge/recipe_kg/__pycache__ && \
