...
随着大语言模型（LLM）与知识增强技术的快速发展，将菜谱知识构建为一个**多模态、结构化、可交互的 AI 系统**成为可能。本项目以中华菜谱数据为基础，构建出覆盖菜谱名称、食材、烹饪步骤、营养成分、菜系流派、历史典故等元素的**多层次知识图谱**，并结合大模型的理解与生成能力，打造了一个**专注于菜谱领域的智能客服**——「SmartRecipe」。
...
| **对话管理** | 完整的对话历史和会话管理 | LangGraph Checkpointer + Redis Stack |

### 技术架构

<div align="center">
  <img src="data/1.png" alt="SmartRecipe Multi-Agent 架构图" width="100%">
  <p><i>图：SmartRecipe Multi-Agent 系统架构总览</i></p>
</div>
...
- **对话管理**: LangGraph Checkpointer 由 Redis Stack 持久化，维护完整会话历史
...
```bash
# 克隆项目
git clone https://github.com/skygazer42/SmartRecipe.git
cd SmartRecipe
...
5. **LangGraph Checkpointer**: 由 Redis Stack 持久化线程态与对话历史
