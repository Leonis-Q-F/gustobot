...
1. 备份原文件:
```bash
cp smartrecipebot/application/agents/lg_prompts.py smartrecipebot/application/agents/lg_prompts.py.backup
```
...
import asyncio
from smartrecipebot.application.agents.lg_states import InputState
from smartrecipebot.application.agents.utils import new_uuid
from smartrecipebot.application.services.runtime_provider import initialize_runtime, get_graph, shutdown_runtime
...
async def test():
    ...
    for query in queries:
        ...
        if state and hasattr(state, "values"):
            router_type = state.values.get('router', {}).get('type')
            print(f"\n查询: {query}")
