"""
Centralised application settings for SmartRecipe.
"""
...
class Settings(BaseSettings):
    ...
    CHECKPOINT_PREFIX: str = Field(
        default="checkpoint",
        description="Redis Stack prefix used by the LangGraph checkpoint store",
    )
    CHECKPOINT_WRITE_PREFIX: str = Field(
        default="checkpoint_write",
        description="Redis Stack prefix used by the LangGraph checkpoint write store",
    )

    # Relational database
    DATABASE_URL: str = "sqlite:///./data/smartrecipebot.db"
    ...
    # Prompt budget / long-memory management
    PROMPT_BUDGET_ENABLED: bool = Field(
        default=True,
        description="Enable prompt budgeting with sliding window and dynamic summaries",
    )
    PROMPT_BUDGET_CONTEXT_WINDOW: int = Field(
        default=131072,
        description="Model context window used by the prompt budget manager",
    )
    PROMPT_BUDGET_OUTPUT_RESERVE: int = Field(
        default=8192,
        description="Reserved output budget for the final model response",
    )
    PROMPT_BUDGET_SAFETY_MARGIN_RATIO: float = Field(
        default=0.10,
        description="Safety margin ratio used to absorb tokenizer/provider drift",
    )
    PROMPT_BUDGET_TOKENIZER: str = Field(
        default="cl100k_base",
        description="Fallback tiktoken encoding when model-specific tokenizer is unavailable",
    )
    PROMPT_BUDGET_RECENT_SOFT_LIMIT: int = Field(
        default=12288,
        description="Recent-window token limit that triggers compaction",
    )
    PROMPT_BUDGET_RECENT_TARGET_LIMIT: int = Field(
        default=8192,
        description="Recent-window token target after compaction",
    )
    PROMPT_BUDGET_RECENT_HARD_LIMIT: int = Field(
        default=16384,
        description="Hard recent-window limit enforced after each completed turn",
    )
    PROMPT_BUDGET_SUMMARY_MAX_TOKENS: int = Field(
        default=2048,
        description="Maximum tokens retained in rolling summaries",
    )
    PROMPT_BUDGET_FACT_MAX_TOKENS: int = Field(
        default=768,
        description="Maximum tokens retained in fact memory",
    )
    PROMPT_BUDGET_REDIS_PREFIX: str = Field(
        default="prompt_memory",
        description="Redis prefix used by the prompt memory store",
    )
