from smartrecipebot.application.agents.lg_prompts import (
...
from smartrecipebot.application.agents.lg_states import AgentState, InputState, Router, GradeHallucinations
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.retrievers.cypher_examples.recipe_retriever import \
    RecipeCypherRetriever
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.planner.node import create_planner_node
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.workflows.multi_agent.multi_tool import (
    create_multi_tool_workflow,
    create_kb_multi_tool_workflow,
)
from smartrecipebot.application.agents.kg_sub_graph.kg_neo4j_conn import get_neo4j_graph
from pydantic import BaseModel, Field
from langchain_core.messages import AIMessage
from langchain_core.messages import messages_to_dict
from langchain_core.runnables.base import Runnable
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.utils.utils import \
...
from smartrecipebot.application.agents.kb_tools import create_knowledge_query_node, KnowledgeQueryInputState
from smartrecipebot.infrastructure.knowledge import KnowledgeService
from smartrecipebot.application.prompt_budget import (
    PromptBudgetManager,
    render_fact_memory,
...
def _build_prompt_budget_manager(*, summary_llm: Optional[BaseChatModel] = None) -> PromptBudgetManager:
    return PromptBudgetManager(summary_llm=summary_llm)


def _recent_messages_from_state(state: AgentState) -> List[Dict[str, Any]]:
    prompt_memory = getattr(state, "prompt_memory", {}) or {}
    recent_messages = prompt_memory.get("recent_messages")
    if isinstance(recent_messages, list) and recent_messages:
        return list(recent_messages)

    messages = [
        msg
        for msg in getattr(state, "messages", [])[:-1]
        if getattr(msg, "type", None) in {"human", "ai"} and getattr(msg, "content", "")
    ]
    return list(messages_to_dict(messages))


def _memory_summary_from_state(state: AgentState) -> Dict[str, Any]:
    prompt_memory = getattr(state, "prompt_memory", {}) or {}
    value = prompt_memory.get("rolling_summary")
    return value if isinstance(value, dict) else {}


def _fact_memory_from_state(state: AgentState) -> Dict[str, Any]:
    prompt_memory = getattr(state, "prompt_memory", {}) or {}
    value = prompt_memory.get("fact_memory")
    return value if isinstance(value, dict) else {}


def _render_history_context(memory_state: Dict[str, Any], *, recent_turn_limit: int = 4) -> str:
    history_entries = serialize_recent_history(memory_state, recent_turn_limit=recent_turn_limit)
    if not history_entries:
        return "（无历史上下文）"
    return "\n".join(f"{item.get('role', 'user')}: {item.get('content', '')}" for item in history_entries)
...
async def respond_to_general_query(
    ...
    if settings.PROMPT_BUDGET_ENABLED:
        ...
        manager = _build_prompt_budget_manager(summary_llm=model)
        prepared = await manager.prepare(
            ...
            recent_messages=_recent_messages_from_state(state),
            rolling_summary=_memory_summary_from_state(state),
            fact_memory=_fact_memory_from_state(state),
            retrieval_segments=None,
            memory_state=getattr(state, "prompt_memory", {}) or {},
        )
        response = await model.ainvoke(prepared.messages)
        return {"messages": [response], "prompt_memory": prepared.updated_memory_state}
...
async def create_kb_query(
    ...
    try:
        ...
        memory_state = getattr(state, "prompt_memory", {}) or {}
        if settings.PROMPT_BUDGET_ENABLED:
            manager = _build_prompt_budget_manager(summary_llm=llm)
            prepared = await manager.prepare(
                route="kb-query",
                session_id=str(config_opts.get("thread_id") or _extract_configurable(config).get("thread_id") or ""),
                system_prompt="你是菜谱文化知识检索的会话记忆压缩器，只负责整理历史，不直接回答用户问题。",
            ...
            memory_state = prepared.updated_memory_state
            history_payload = serialize_recent_history(memory_state, recent_turn_limit=4)
        else:
            history_payload = [
                {
        ...
        return {"messages": [ai_message], "sources": sources, "prompt_memory": memory_state}
    except Exception as exc:
...
async def create_research_plan(
    ...
    #  定义工具模式列表
    from smartrecipebot.application.agents.kg_sub_graph.kg_tools_list import (
        cypher_query,
        predefined_cypher,
    ...
    from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.predefined_cypher.cypher_dict import \
    ...
    if settings.PROMPT_BUDGET_ENABLED:
        ...
        prepared = await manager.prepare(
            ...
            system_prompt="你是图谱/统计工作流的会话记忆压缩器，只负责整理历史，不直接回答用户问题。",
            current_user_message=last_message,
            recent_messages=_recent_messages_from_state(state),
            rolling_summary=_memory_summary_from_state(state),
            fact_memory=_fact_memory_from_state(state),
        ...
        prompt_memory = prepared.updated_memory_state
        history_context = _render_history_context(prompt_memory, recent_turn_limit=4)
    ...
    return {"messages": [AIMessage(content=response["answer"])], "prompt_memory": prompt_memory}
...
def build_agent_graph(checkpointer: Any) -> CompiledStateGraph:
    """使用外部注入的 checkpointer 编译主聊天图。"""
