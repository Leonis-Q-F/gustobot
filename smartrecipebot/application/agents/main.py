...
from smartrecipebot.application.services.runtime_provider import (
    get_chat_runtime,
    get_graph,
    initialize_runtime,
    shutdown_runtime,
...
def _resolve_memory_turn_limit() -> Optional[int]:
    ...
    raw_value = os.getenv("SMARTRECIPE_MEMORY_TURNS", os.getenv("SMARTRECIPE_MAX_MEMORY_TURNS", "2"))
    try:
        value = int(raw_value)
...
async def process_query(query: str) -> None:
    ...
    if settings.PROMPT_BUDGET_ENABLED:
        async for chunk, metadata in chat_runtime.astream_messages(
            query=query,
            config=thread,
        ):
        ...
        state_snapshot = await graph.aget_state(thread)
