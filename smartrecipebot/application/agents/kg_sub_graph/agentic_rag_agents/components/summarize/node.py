...
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.state import OverallState
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.summarize.prompts import create_summarization_prompt_template
    ...
    async def summarize(state: OverallState) -> Dict[str, Any]:
        ...
        try:
            summary = await generate_summary.ainvoke(
                {
                    "question": state.get("question", "") or "未提供用户问题",
                    "results": factual_results,
                }
            )
            summary = summary.strip()
        except Exception as exc:  # pragma: no cover - defensive fallback
            logger.error(f"LLM 总结失败，回退到规则摘要: {exc}", exc_info=True)
            summary = factual_results

        if not summary:
            summary = factual_results or "暂时没有可总结的数据。"
