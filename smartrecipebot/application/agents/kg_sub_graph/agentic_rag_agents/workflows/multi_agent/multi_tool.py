...
# 导入输入输出状态定义
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.state import (
    InputState,
    OutputState,
    OverallState,
)
# 导入guardrails逻辑
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.guardrails.node import create_guardrails_node
# 导入分解节点
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.planner import create_planner_node
# 导入工具选择节点
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.tool_selection import create_tool_selection_node
# 导入 text2cypher 节点
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.cypher_tools import create_cypher_query_node
# 导入Cypher示例检索器基类
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.retrievers.cypher_examples.base import BaseCypherExampleRetriever
# 导入预定义Cypher节点
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.predefined_cypher import create_predefined_cypher_node
# 导入自定义工具函数节点
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.customer_tools import create_graphrag_query_node
from smartrecipebot.application.agents.kg_sub_graph.agentic_rag_agents.components.text2cypher.text2sql_tool import create_text2sql_tool_node
...
from smartrecipebot.application.prompt_budget import PromptSegment, TokenCounter, resolve_route_budget
from smartrecipebot.application.prompt_budget.assembler import pack_retrieval_segments, render_retrieval_segments
...
def create_kb_multi_tool_workflow(
    ...
    *,
    ...
    kb_policy = resolve_route_budget("kb-query")
    ...
    def _history_to_text(history: List[Dict[str, str]], limit: int = 4) -> str:
        ...
        for item in history:
            ...
            if role == "system" and content:
                summary_segments.append(f"{role}: {content}")
                continue
            recent_segments.append(f"{role}: {content}")
        segments: List[str] = []
        if summary_segments:
            segments.extend(summary_segments[-1:])
        segments.extend(recent_segments[-limit:])
        return "\n".join(segments)
    ...
    def _format_results(
        ...
        *,
        ...
        packed_segments, _ = pack_retrieval_segments(
            segments,
        ...
        if not packed_segments:
            return empty_hint
        return render_retrieval_segments(packed_segments)
    ...
    def _format_milvus_results(results: List[Dict[str, Any]]) -> str:
        return _format_results(
            ...
            budget_tokens=kb_policy.retrieval_budget // 3,
        )
    ...
    def _format_postgres_results(results: List[Dict[str, Any]]) -> str:
        return _format_results(
            ...
            budget_tokens=kb_policy.retrieval_budget // 2,
        )
    ...
    def _format_combined_local_results(results: List[Dict[str, Any]]) -> str:
        return _format_results(
            ...
            default_label="本地",
            empty_hint="（无本地检索结果）",
            budget_tokens=(kb_policy.retrieval_budget * 2) // 3,
        )
    ...
    def _format_external_results(results: List[Dict[str, Any]]) -> str:
        ...
        packed_segments, _ = pack_retrieval_segments(
            segments,
            counter=token_counter,
            budget_tokens=kb_policy.retrieval_budget // 4,
        )
        if not packed_segments:
            return "（无外部检索结果）"
        return render_retrieval_segments(packed_segments)
